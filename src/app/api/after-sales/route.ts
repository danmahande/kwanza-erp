...
export async function POST(req: NextRequest) {
  try {
    ...
    // ===========================================================================
    // Workflow 3: Returns disposition (unit-level)
    // When a customer returns goods, we record which specific itemIds came back
    // and (later) what to do with each: RESTOCK / RTV / DISPOSE / LIQUIDATE.
    ...
    const afterSalesRecord = await db.afterSalesRecord.create({
      data: {
        ...
        dispositions: null, // dispositions are decided later, on approval
      },
    })

    // Workflow 3: log a RETURNED_TO_WAREHOUSE event on each returned item
    // (non-blocking — InventoryItem table may not exist)
    try {
...
export async function PUT(req: NextRequest) {
  try {
    ...
    // ===========================================================================
    // Workflow 3: On approval, apply per-item dispositions.
    ...
    if (Array.isArray(body.dispositions) && body.dispositions.length > 0) {
          ...
          if (d.disposition === 'RTV') {
            ...
            if (item) {
              ...
              await db.rTVRecord.create({
                data: {
                  rtvId,
                  originalOrderId: afterSalesRecord.originalOrderId,
                  returnOrderNumber: afterSalesRecord.returnOrderNumber,
      ...
      await db.afterSalesRecord.update({
        where: { id },
        data: { dispositions: body.dispositions },
      })
    }
