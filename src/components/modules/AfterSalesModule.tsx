...
const emptyForm = { originalOrderId: '', customerId: '', customerName: '', reason: '', refundAmount: '', itemIds: '' }
...
export default function AfterSalesModule() {
  ...
  const handleSubmit = async () => {
    ...
    const itemIds = form.itemIds.split(',').map(s => s.trim()).filter(Boolean)
    const payload = {
  ...
  const handleApprove = async () => {
    ...
    try {
      const res = await fetch('/api/after-sales', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: editing.id, returnStatus: 'approved', dispositions }),
      })
      if (res.ok) {
          ...
          <div className="min-h-full flex flex-col">
            <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
              <div className="px-6 py-3 flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-base font-bold text-gray-900">New RMA (Customer Return)</h1>
                  <p className="text-[11px] text-gray-500">The original order # will be flipped from DS to RT automatically</p>
                </div>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto">
              <div className="max-w-2xl mx-auto px-6 py-8 space-y-5">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Select the original order (optional), enter the customer and reason, and scan the returned item barcodes.</p>
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={e => {
                      ...
                      setForm({ ...form, originalOrderId: e.target.value, customerName: order?.customerName || form.customerName })
                    }}
            ...
            <div className="bg-white border-t border-gray-200 sticky bottom-0">
              <div className="max-w-2xl mx-auto px-6 py-3 flex items-center justify-end gap-2">
                ...
                <Button size="sm" className="bg-[#FF6B35] hover:bg-[#E55A25] text-white rounded-xl" onClick={handleSubmit}>Create RMA</Button>
              </div>
            </div>
          </div>
        </PageTransition>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Button size="sm" className="h-8 text-xs rounded-md bg-[#FF6B35] hover:bg-[#E55A25] text-white" onClick={openCreate}>
              <Plus size={12} className="mr-1" /> New RMA
            </Button>
          </div>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                    isActive ? 'bg-[#FF6B35] text-white shadow-sm' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
                  }`}
                >
                  {chip.label}
          ...
          {filteredRmas.length === 0 ? (
            <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
              <div className="w-16 h-16 mx-auto bg-orange-50 rounded-full flex items-center justify-center mb-4">
                <PackageX size={28} className="text-orange-500" />
              </div>
              <h3 className="text-base font-bold text-gray-900 mb-1">No RMAs match this filter</h3>
              <p className="text-xs text-gray-500 max-w-md mx-auto mb-4">Try a different filter or create a new RMA.</p>
              <Button className="bg-[#FF6B35] hover:bg-[#E55A25] text-white rounded-xl" onClick={openCreate}>
                <Plus size={14} className="mr-1.5" /> New RMA
              </Button>
            </div>
          ) : (
            <div>
          ...
          {/* Disposition dialog (stays as dialog — quick action modal) */}
